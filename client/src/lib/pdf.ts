...
export const generateInvoicePDF = async (transaction: Transaction) => {
  ...
  // Header
  doc.setFillColor(15, 76, 129); // Blue header
  doc.rect(0, 0, 210, 40, 'F');
  
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(24);
  doc.text('INWORA DEMO', 20, 25);
  doc.setFontSize(12);
  ...
  yPos += 20;
  doc.setFontSize(10);
  doc.text('Thank you for choosing INWORA DEMO!', 20, yPos);
  doc.text('For support, contact us at support@inwora-demo.com', 20, yPos + 7);
  
  doc.save(`inwora-demo-invoice-${transaction.id.slice(-6)}.pdf`);
};
...
export const generateDailySummaryPDF = async (summary: DailySummary, transactions: Transaction[]) => {
  ...
  doc.save(`inwora-demo-daily-summary-${summary.date}.pdf`);
};
...
export const generateWeeklySummaryPDF = async (summary: WeeklySummary, transactions: Transaction[]) => {
  ...
  doc.save(`inwora-demo-weekly-summary-${summary.weekStart}.pdf`);
};
...
export const generateMonthlySummaryPDF = async (summary: MonthlySummary, transactions: Transaction[]) => {
  ...
  doc.save(`inwora-demo-monthly-summary-${summary.month}.pdf`);
};
...
export const generateMenuSalesPDF = async (menuSales: Array<{name: string, count: number}>) => {
  ...
  doc.setFontSize(10);
  doc.text('Generated by INWORA DEMO Analytics', 20, yPos);
  doc.text(`Report generated on: ${new Date().toLocaleString()}`, 20, yPos + 7);
  
  const filename = `inwora-demo-menu-sales-${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(filename);
};
