...
const addFooter = (doc: jsPDF, yPosition: number) => {
  ...
  doc.text('Thank you for choosing INWORA DEMO!', 20, footerY);
  doc.text('For support, contact us at support@inwora-demo.com', 20, footerY + 6);
...
export const generateInvoicePDF = async (transaction: Transaction) => {
  ...
  // Save the PDF
  doc.save(`inwora-demo-invoice-${transaction.id.slice(-8)}.pdf`);
};
